...
public class PolicyAlterationUtility {
	...
	public static EndorseKeyAndValue createMapping(Integer lsje_id, String key,DetailPolicyAlteration alteration, VegaServices services) {
		...
		if(m != null) {
			...
			if(m.getIs_old_value_string()>0) {
				endorseKeyAndValue.setIsOldKeyString(true);
				try {
					String query = m.getOld_value_string();
					String value = alteration.getOld();
					query = query.replaceAll("#VALUE#", "'"+value.trim()+"'");
					String result = services.selectDynamicQuery(new SQLAdapter(query));
					endorseKeyAndValue.setOldKeyString(result);
				}catch(Exception e) {
					endorseKeyAndValue.setOldKeyString(e.getMessage());
